...
use social_media_core::token::DEFAULT_TOKEN_TTL_SECONDS;
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        for (name, secret) in [("JWT_SECRET", &jwt_secret), ("MEDIA_TOKEN_SECRET", &media_token_secret)] {
            if secret.len() < 32 {
                return Err(ConfigError::Invalid {
                    name,
        ...
        .map_err(|error| ConfigError::Invalid { name: "STUN_URL/TURN_URL", reason: error.to_string() })?;
