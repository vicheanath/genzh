...
use serde::Serialize;
use social_auth::AuthError;
use social_domain::DomainError;
use social_infrastructure::ServiceError;
    ...
    fn parts(&self) -> (StatusCode, String, String) {
        match self {
            ...
            ApiError::BadRequest(message) => {
                (StatusCode::BAD_REQUEST, "BAD_REQUEST".to_owned(), message.clone())
            }
...
fn domain_parts(error: &DomainError) -> (StatusCode, String, String) {
    match error {
        DomainError::Invalid { .. } | DomainError::UnknownPermission(_) => {
            (StatusCode::BAD_REQUEST, error.code().to_owned(), error.to_string())
        }
        DomainError::NotFound(_) => {
            (StatusCode::NOT_FOUND, error.code().to_owned(), error.to_string())
        }
        DomainError::Conflict(_) => {
            (StatusCode::CONFLICT, error.code().to_owned(), error.to_string())
        }
...
mod tests {
    ...
    fn storage_failures_never_reach_the_client() {
        let error = ApiError::Service(ServiceError::Repository(
            social_infrastructure::RepositoryError::Migration("column x does not exist".into()),
        ));
        let (status, code, message) = error.parts();
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);
        assert_eq!(code, "INTERNAL_ERROR");
        assert!(!message.contains("column"), "internal detail leaked: {message}");
    }
    ...
    fn validation_failures_are_400_and_keep_their_detail() {
        let error = ApiError::Domain(DomainError::invalid("handle", "must be at least 3 characters"));
        let (status, code, message) = error.parts();
    ...
    fn the_error_body_has_the_documented_shape() {
        ...
        assert_eq!(json.as_object().unwrap().len(), 1, "no stray top-level fields");
    }

    #[test]
    fn not_found_and_conflict_map_to_their_status_codes() {
        assert_eq!(ApiError::Domain(DomainError::NotFound("room")).parts().0, StatusCode::NOT_FOUND);
        assert_eq!(
            ApiError::Domain(DomainError::Conflict("membership")).parts().0,
            StatusCode::CONFLICT
        );
    }
}
