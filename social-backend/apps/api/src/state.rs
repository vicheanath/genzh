...
use social_auth::{AuthService, JwtService};
use social_community::CommunityService;
use social_graph::SocialService;
use social_infrastructure::{DbPool, PgConfig, RepositoryResult, connect};
use social_media_core::token::MediaTokenSigner;
use social_messaging::MessagingService;
use social_room::{MediaSessionService, RoomService, StaticMediaServers};
...
impl AppState {
    ...
    pub async fn build(config: Config) -> RepositoryResult<Self> {
        ...
        let servers = Arc::new(StaticMediaServers::from_env_value(&config.media_server_urls));
