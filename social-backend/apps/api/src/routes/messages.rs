...
use social_domain::message::{Message, ReactionTally};
use social_domain::{MessageId, RoomId};
...
pub async fn list(
    ...
    let page = state
        ...
        .await?;
    Ok(Json(HistoryResponse { messages: page.messages, next_before: page.next_before }))
}
...
pub async fn post(
    ...
    let message = state.messaging.post(room_id, caller.user_id, &body.content).await?;
...
pub async fn edit(
    ...
    Ok(Json(state.messaging.edit(message_id, caller.user_id, &body.content).await?))
}
...
pub async fn react(
    ...
    Ok(Json(state.messaging.react(message_id, caller.user_id, &body.reaction).await?))
}
...
pub async fn unreact(
    ...
    Ok(Json(state.messaging.unreact(message_id, caller.user_id, &body.reaction).await?))
}
