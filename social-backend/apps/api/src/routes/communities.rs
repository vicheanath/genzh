...
use social_domain::community::{Community, CommunityMember, RoleWithPermissions};
use social_domain::{CommunityId, Permission, RoleId, UserId};
...
pub async fn create(
    ...
    let community = state
        ...
        .create(
            caller.user_id,
            social_community::CreateCommunity {
                name: body.name,
...
pub async fn get(
    ...
    let context = state.communities.member_context(community_id, caller.user_id).await?;
...
pub async fn update(
    ...
    let community = state
        ...
        .update(
            community_id,
            caller.user_id,
            social_community::UpdateCommunity {
                name: body.name,
                description: body.description,
                icon_url: body.icon_url,
            },
...
pub async fn delete(
    ...
    state.communities.delete(community_id, caller.user_id).await?;
...
pub async fn add_member(
    ...
    let member = state.communities.add_member(community_id, caller.user_id, target).await?;
...
pub async fn remove_member(
    ...
    state.communities.remove_member(community_id, caller.user_id, user_id).await?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn list_roles(
    ...
    Ok(Json(state.communities.list_roles(community_id, caller.user_id).await?))
}
...
pub async fn update_role(
    ...
    let permissions = body.permissions.as_deref().map(parse_permissions).transpose()?;
...
mod tests {
    ...
    fn an_unknown_permission_key_is_a_client_error_not_a_silent_drop() {
        ...
        assert!(matches!(
            error,
            ApiError::Domain(social_domain::DomainError::UnknownPermission(key)) if key == "become_root"
        ));
    }
