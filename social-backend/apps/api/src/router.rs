...
pub fn build(state: AppState) -> Router {
    ...
    let api = Router::new()
        ...
        .route("/me", get(routes::auth::me).patch(routes::auth::update_profile))
        // ---- communities ----
        ...
        .route("/communities/{id}/roles/{role_id}", patch(routes::communities::update_role))
        // ---- rooms ----
        .route(
        ...
        .route(
            "/rooms/{id}",
            get(routes::rooms::get).patch(routes::rooms::update).delete(routes::rooms::delete),
        )
        // ---- media ----
        ...
        // ---- social ----
        .route("/friends", get(routes::social::list).post(routes::social::request))
        ...
        .route("/blocks/{user_id}", put(routes::social::block).delete(routes::social::unblock));
...
fn cors_layer(allowed: &str) -> CorsLayer {
    ...
    if origins.is_empty() { base.allow_origin(Any) } else { base.allow_origin(origins) }
}
