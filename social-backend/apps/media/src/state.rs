...
use social_media_room::{MediaRoomManager, PeerFactory, RoomConfig, SfuConfig};
use social_media_room::room::AutoSubscribe;
...
impl MediaState {
    ...
    pub fn build(config: Config) -> Result<Self, social_media_room::MediaRoomError> {
        ...
        Ok(Self { rooms, peers, verifier, config: Arc::new(config) })
    }
}
