...
mod tests {
    ...
    fn an_access_token_carries_no_authorization_claims() {
        let service = service();
        let token = service.issue_access(UserId::new(), SessionId::new()).expect("issue");
    ...
    fn a_token_signed_with_another_secret_is_rejected() {
        let token = service().issue_access(UserId::new(), SessionId::new()).expect("issue");
        let impostor = JwtService::new(b"some-other-secret-entirely", "social.api", "social.client", 900, 3600);
        assert!(matches!(impostor.authenticate(&token), Err(AuthError::InvalidToken)));
    }
    ...
    fn an_expired_token_is_rejected() {
        ...
        let token = service
            ...
            .expect("issue");
        assert!(matches!(service.authenticate(&token), Err(AuthError::InvalidToken)));
    }

    #[test]
    fn a_token_for_another_audience_is_rejected() {
        let other = JwtService::new(SECRET, "social.api", "someone.elses.client", 900, 3600);
        let token = other.issue_access(UserId::new(), SessionId::new()).expect("issue");
        assert!(matches!(service().authenticate(&token), Err(AuthError::InvalidToken)));
    }
    ...
    fn garbage_is_rejected_without_panicking() {
        ...
        for input in ["", "not.a.token", "....", "Bearer abc"] {
            assert!(matches!(service.authenticate(input), Err(AuthError::InvalidToken)));
        }
    }
    ...
    fn refresh_tokens_are_stored_hashed() {
        ...
        assert_eq!(stored, hash_refresh_token(&token), "hashing is deterministic");
