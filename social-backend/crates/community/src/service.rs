...
use social_domain::community::{self, Community, CommunityMember, Role, RoleWithPermissions};
use social_domain::{
    CommunityId, DomainError, Permission, PermissionSet, RoleId, UserId, now,
};
use social_infrastructure::{DbPool, ServiceError, ServiceResult};
...
impl CommunityService {
    ...
    pub fn new(pool: DbPool) -> Self {
        Self { repository: CommunityRepository::new(pool) }
    }
    ...
    pub async fn member_context(
        ...
        if community.is_owner(user_id) {
            ...
            return Ok(MemberContext::new(&community, user_id, Default::default(), []));
        }

        if self.repository.find_member(community_id, user_id).await?.is_none() {
            return Err(ServiceError::Domain(DomainError::NotAMember));
        }

        let role_ids = self.repository.member_role_ids(community_id, user_id).await?;
        let granted = self.repository.member_permissions(community_id, user_id).await?;
    ...
    pub async fn update(
        ...
        let name = input.name.as_deref().map(community::validate_community_name).transpose()?;
    ...
    pub async fn add_member(
        ...
        if self.repository.find_member(community_id, target_id).await?.is_some() {
            return Err(ServiceError::Domain(DomainError::Conflict("membership")));
        }

        Ok(self.repository.add_member(community_id, target_id, None).await?)
    }
    ...
    pub async fn remove_member(
        ...
        if !self.repository.remove_member(community_id, target_id).await? {
    ...
    pub async fn list_members(
        ...
        self.member_context(community_id, user_id).await?;
        Ok(self.repository.list_members(community_id, limit.clamp(1, 200)).await?)
    }
    ...
    pub async fn update_role(
        ...
        let name = input.name.as_deref().map(community::validate_role_name).transpose()?;
        ...
        let effective = match permissions {
            ...
            None => PermissionSet::from_permissions(
                self.repository.role_permissions(role_id).await?,
            ),
        };

        Ok(RoleWithPermissions { role: updated, permissions: effective })
    }
    ...
    pub async fn assign_role(
        ...
        if self.repository.find_role(community_id, role_id).await?.is_none() {
            return Err(ServiceError::not_found("role"));
        }
        if self.repository.find_member(community_id, target_id).await?.is_none() {
            return Err(ServiceError::not_found("membership"));
        }
        ...
        self.repository.assign_role(community_id, target_id, role_id).await?;
        Ok(())
    }
...
fn guard_privilege_escalation(
    ...
    for permission in &requested {
        if !context.allows(*permission) {
            return Err(ServiceError::Domain(DomainError::PermissionDenied(permission.key())));
        }
    }
    Ok(requested)
}
...
mod tests {
    ...
    fn a_moderator_cannot_mint_a_role_more_powerful_than_themselves() {
        ...
        let escalation =
            guard_privilege_escalation(&moderator, vec![Permission::Administrator]);
        assert!(matches!(
            escalation,
            Err(ServiceError::Domain(DomainError::PermissionDenied("administrator")))
        ));
