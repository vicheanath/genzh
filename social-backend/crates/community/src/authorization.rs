...
use social_domain::{CommunityId, DomainError, DomainResult, Permission, PermissionSet, RoleId, UserId};
...
pub fn apply_room_overrides(
    ...
    for entry in overrides.iter().filter(|o| member_roles.contains(&o.role_id)) {
        if entry.allow {
...
mod tests {
    ...
    fn the_owner_gets_everything_without_holding_a_single_role() {
        ...
        for permission in Permission::ALL {
            assert!(context.allows(*permission), "owner should hold {permission}");
        }
    }
    ...
    fn a_member_holds_the_union_of_their_roles() {
        ...
        let context = MemberContext::new(
            ...
            member,
            HashSet::new(),
            [Permission::ViewRoom, Permission::SendMessage, Permission::Speak],
        );
    ...
    fn require_returns_the_permission_that_was_missing() {
        let context =
            MemberContext::new(&community(UserId::new()), UserId::new(), HashSet::new(), [Permission::ViewRoom]);
    ...
    fn a_room_denial_removes_a_community_permission() {
        ...
        let stage = apply_room_overrides(
            base,
            &roles,
            &[RoomOverride { role_id: role, permission: Permission::Speak, allow: false }],
        );

        assert!(!stage.allows(Permission::Speak));
        assert!(stage.allows(Permission::ViewRoom), "unrelated permissions are untouched");
    }
    ...
    fn a_room_grant_adds_a_permission_the_member_lacks_elsewhere() {
        ...
        let presenting = apply_room_overrides(
            ...
            &roles,
            &[RoomOverride { role_id: role, permission: Permission::ScreenShare, allow: true }],
        );
    ...
    fn an_allow_on_one_role_beats_a_deny_on_another() {
        ...
        let resolved = apply_room_overrides(
            ...
            &roles,
            &[
                RoomOverride { role_id: muted_role, permission: Permission::Speak, allow: false },
                RoomOverride { role_id: speaker_role, permission: Permission::Speak, allow: true },
            ],
        );

        assert!(resolved.allows(Permission::Speak), "the speaker role should win");
    }
    ...
    fn overrides_for_roles_the_member_does_not_hold_are_ignored() {
        ...
        let resolved = apply_room_overrides(
            PermissionSet::default_member(),
            &HashSet::from([held]),
            &[RoomOverride { role_id: not_held, permission: Permission::Speak, allow: false }],
        );
    ...
    fn a_room_override_cannot_lock_out_an_administrator() {
        ...
        let resolved = apply_room_overrides(
            ...
            &HashSet::from([role]),
            &[
                RoomOverride { role_id: role, permission: Permission::ViewRoom, allow: false },
                RoomOverride { role_id: role, permission: Permission::Speak, allow: false },
            ],
        );
