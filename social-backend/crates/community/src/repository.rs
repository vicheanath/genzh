...
use social_domain::community::{Community, CommunityMember, Role};
...
impl CommunityRepository {
    ...
    pub async fn create(
        ...
        .bind(community.id)
        .bind(social_domain::community::EVERYONE_ROLE_NAME)
        .execute(&mut *tx)
        .await?;
    ...
    pub async fn member_permissions(
        ...
        Ok(rows.into_iter().filter_map(|row| row.permission_key.parse().ok()).collect())
    }
