...
impl MediaTokenSigner {
    ...
    pub fn issue_at(&self, grant: &MediaGrant, now: DateTime<Utc>) -> MediaCoreResult<MediaToken> {
        ...
        let token =
            encode(&self.header, &claims, &self.encoding).map_err(MediaCoreError::Sign)?;

        Ok(MediaToken { token, participant_id: grant.participant_id, expires_at })
    }
...
mod tests {
    ...
    fn a_token_signed_with_another_secret_is_rejected() {
        let minted = signer().issue(&grant()).expect("issue");
        let impostor = MediaTokenSigner::new(b"a-completely-different-secret-value", "social.api", 120);
    ...
    fn an_expired_token_is_rejected() {
        ...
        assert!(signer.verify(&minted.token).is_err(), "expired token must not verify");
    }
    ...
    fn a_token_from_the_future_is_rejected() {
        ...
        assert!(signer.verify(&minted.token).is_err(), "nbf in the future must not verify");
    }
    ...
    fn ttl_is_clamped_into_a_sane_band() {
        ...
        assert_eq!(MediaTokenSigner::new(SECRET, "i", 86_400).ttl_seconds(), 600);
