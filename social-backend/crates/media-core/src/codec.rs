...
//! particular WebRTC library. `social-media-room` translates these profiles
...
impl From<TrackKind> for MediaKind {
    fn from(kind: TrackKind) -> Self {
        if kind.is_audio() { MediaKind::Audio } else { MediaKind::Video }
    }
}
...
impl CodecRegistry {
    ...
    pub fn allows_mime(&self, mime_type: &str) -> bool {
        self.codecs.iter().any(|c| c.mime_type.eq_ignore_ascii_case(mime_type))
    }
...
mod tests {
    ...
    fn the_default_registry_carries_audio_and_video() {
        ...
        assert!(registry.allows_mime("video/vp8"), "mime matching is case-insensitive");
    }
    ...
    fn an_absent_list_means_everything() {
        assert_eq!(CodecRegistry::from_allow_list(None), CodecRegistry::default());
        assert_eq!(CodecRegistry::from_allow_list(Some("   ")), CodecRegistry::default());
    }
    ...
    fn payload_types_do_not_collide() {
        ...
        for codec in ALL_CODECS {
            assert!(!seen.contains(&codec.payload_type), "duplicate PT {}", codec.payload_type);
