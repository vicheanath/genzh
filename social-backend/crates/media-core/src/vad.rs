...
impl Default for AudioLevelVadConfig {
    fn default() -> Self {
        Self { threshold: 45, activation_packets: 3, release_ms: 250 }
    }
}
...
impl AudioLevelVad {
    ...
    pub fn new(config: AudioLevelVadConfig) -> Self {
        Self { config, speaking: false, consecutive_loud: 0, last_loud_ms: 0 }
    }
}
...
mod tests {
    ...
    fn loud(now_ms: u64) -> AudioLevelSample {
        AudioLevelSample { level: Some(20), now_ms }
    }

    fn quiet(now_ms: u64) -> AudioLevelSample {
        AudioLevelSample { level: Some(110), now_ms }
    }
    ...
    fn packets_without_the_extension_are_ignored() {
        let mut vad = AudioLevelVad::default();
        for now_ms in 0..10 {
            assert_eq!(vad.observe(AudioLevelSample { level: None, now_ms }), None);
        }
        assert!(!vad.is_speaking());
    }
    ...
    fn the_mode_selects_the_implementation_and_defaults_safely() {
        assert_eq!(VadMode::from_env_value(Some("server")), VadMode::ServerAudioLevel);
        assert_eq!(VadMode::from_env_value(Some("RTP")), VadMode::ServerAudioLevel);
        assert_eq!(VadMode::from_env_value(Some("client")), VadMode::ClientReported);
        assert_eq!(VadMode::from_env_value(None), VadMode::ClientReported);
        assert_eq!(VadMode::from_env_value(Some("nonsense")), VadMode::ClientReported);
