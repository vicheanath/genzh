...
impl IceServer {
    ...
    pub fn stun(url: impl Into<String>) -> MediaCoreResult<Self> {
        ...
        Ok(Self { urls: vec![url], username: None, credential: None })
    }
...
impl IceConfig {
    ...
    pub fn from_parts(
        ...
        match (turn_url.filter(|u| !u.is_empty()), turn_username, turn_password) {
...
mod tests {
    ...
    fn missing_turn_credentials_downgrade_rather_than_fail() {
        ...
        assert!(!cfg.has_relay(), "an entry without credentials is not a usable relay");
    }
