...
pub fn normalize_handle(raw: &str) -> DomainResult<String> {
    ...
    if handle.starts_with('.') || handle.ends_with('.') {
        return Err(DomainError::invalid("handle", "may not start or end with '.'"));
    }
    Ok(handle)
}
...
pub fn normalize_email(raw: &str) -> DomainResult<String> {
    ...
    if parts.next().is_some() || local.is_empty() || domain.is_empty() || !domain.contains('.') {
        return Err(DomainError::invalid("email", "must be a valid e-mail address"));
    }
    if email.len() > 254 {
        return Err(DomainError::invalid("email", "must be at most 254 characters"));
    }
    Ok(email)
}
...
pub fn validate_password(raw: &str) -> DomainResult<()> {
    ...
    if raw.len() > 1024 {
        return Err(DomainError::invalid("password", "must be at most 1024 bytes"));
    }
    Ok(())
}
