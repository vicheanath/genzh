...
use social_media_core::codec::{CodecProfile, CodecRegistry, MediaKind};
use social_media_core::ice::{IceConfig, IceTransportPolicy};
use social_media_core::track::{ParticipantId, TrackId, TrackKind};
use social_media_core::vad::{AudioLevelSample, SpeakingTransition, VadMode, VoiceActivityDetector};
use social_media_signaling::PeerTarget;
...
impl std::fmt::Debug for PeerFactory {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PeerFactory").field("config", &self.config).finish_non_exhaustive()
    }
}
...
impl PeerFactory {
    ...
    pub fn new(config: SfuConfig) -> MediaRoomResult<Arc<Self>> {
        ...
        Ok(Arc::new(Self { config: Arc::new(config), runtime }))
    }
    ...
    pub async fn create(
        ...
        let subscriber = self
            .build(Arc::new(SubscriberHandler { events: event_tx.clone() }))
            .await?;
        ...
        Ok((peers, PeerEvents { events: event_rx, renegotiate: renegotiate_rx }))
    }
}
...
impl ParticipantPeers {
    ...
    pub async fn accept_publisher_offer(&self, sdp: String) -> MediaRoomResult<String> {
        ...
        self.publisher.set_remote_description(offer).await.map_err(MediaRoomError::from)?;

        let answer = self.publisher.create_answer(None).await.map_err(MediaRoomError::from)?;
        let sdp = answer.sdp.clone();
        self.publisher.set_local_description(answer).await.map_err(MediaRoomError::from)?;
    ...
    pub async fn create_subscriber_offer(&self) -> MediaRoomResult<Option<String>> {
        if self.negotiation.awaiting_answer.swap(true, Ordering::SeqCst) {
        ...
        let offer = match self.subscriber.create_offer(None).await {
            Ok(offer) => offer,
            Err(error) => {
                self.negotiation.awaiting_answer.store(false, Ordering::SeqCst);
    ...
    pub async fn accept_subscriber_answer(&self, sdp: String) -> MediaRoomResult<bool> {
        ...
        self.subscriber.set_remote_description(answer).await.map_err(MediaRoomError::from)?;
    ...
    pub async fn add_ice_candidate(
        ...
        peer.add_ice_candidate(init).await.map_err(MediaRoomError::from)
    }
...
impl PeerConnectionEventHandler for PublisherHandler {
    ...
    async fn on_connection_state_change(&self, state: RTCPeerConnectionState) {
        let _ = self
            .events
            .send(PeerEvent::ConnectionState { target: PeerTarget::Publisher, state })
            .await;
    }
    ...
    async fn on_track(&self, track: Arc<dyn TrackRemote>) {
        ...
        let published = PublishedTrack::with_codec(
            ...
            kind,
            codec,
            Some(keyframe_requester(track.clone(), ssrc, self.runtime.clone())),
        );

        self.spawn_pump(track, published.clone(), kind);

        if self.events.send(PeerEvent::TrackReady { track: published }).await.is_err() {
            tracing::debug!(
                participant_id = %self.participant_id,
...
impl PublisherHandler {
    ...
    fn spawn_pump(&self, remote: Arc<dyn TrackRemote>, published: Arc<PublishedTrack>, kind: TrackKind) {
            ...
            while let Some(event) = remote.poll().await {
                match event {
                    TrackRemoteEvent::OnRtpPacket(mut packet) => {
                        ...
                        if kind.is_audio()
                            && let Some(transition) = observe_audio_level(
                                vad.as_mut(),
                                &packet,
                                audio_level_ext_id,
                            )
...
impl PeerConnectionEventHandler for SubscriberHandler {
    ...
    async fn on_connection_state_change(&self, state: RTCPeerConnectionState) {
        let _ = self
            .events
            .send(PeerEvent::ConnectionState { target: PeerTarget::Subscriber, state })
            .await;
    }
}
...
impl SubscriberSink for WebRtcSubscriberSink {
    async fn attach(&self, track: Arc<PublishedTrack>) -> MediaRoomResult<()> {
        ...
        let kind = if track.kind().is_audio() { RtpCodecKind::Audio } else { RtpCodecKind::Video };
        ...
        self.subscriptions
            .lock()
            .await
            .insert(track_id, Subscription { sender, forward, feedback });
    ...
    async fn close(&self) {
        let subscriptions: Vec<Subscription> =
            self.subscriptions.lock().await.drain().map(|(_, s)| s).collect();
...
fn resolve_subscriber_codec(
    ...
    codecs
        .for_kind(media_kind)
        .find(|profile| profile.mime_type.eq_ignore_ascii_case(&track.info().mime_type))
...
pub(crate) fn observe_audio_level(
    ...
    vad.observe(AudioLevelSample { level, now_ms: monotonic_millis() })
}
...
pub fn connection_state_is_terminal(state: RTCPeerConnectionState) -> bool {
    matches!(state, RTCPeerConnectionState::Failed | RTCPeerConnectionState::Closed)
}
...
async fn emit_candidate(
    ...
    let Ok(init) = event.candidate.to_json() else {
        tracing::debug!(target = target.as_str(), "could not serialise local ICE candidate");
        return;
    };
...
mod tests {
    use super::*;
    use social_media_core::vad::{AudioLevelVad, NoopVad};
    ...
    fn stripping_a_packet_without_extensions_is_a_no_op() {
        ...
        strip_header_extensions(&mut packet);
        assert_eq!(packet.header.sequence_number, 42, "the rest of the header is untouched");
    }
    ...
    fn the_payload_survives_stripping() {
        ...
        strip_header_extensions(&mut packet);
        assert_eq!(&packet.payload[..], b"opus-frame-bytes", "the SFU must not touch media");
    }
    ...
    fn the_voice_activity_bit_is_masked_off_the_level() {
        ...
        assert!(vad.is_speaking(), "the top bit must not be read as part of the level");
    }
    ...
    fn codec_profiles_translate_into_engine_parameters() {
        let parameters = codec_parameters(&social_media_core::codec::OPUS);
        ...
        assert!(parameters.rtp_codec.sdp_fmtp_line.contains("useinbandfec=1"));
    }
    ...
    fn a_network_handover_does_not_end_the_session() {
        ...
        assert!(!connection_state_is_terminal(RTCPeerConnectionState::Disconnected));
        assert!(!connection_state_is_terminal(RTCPeerConnectionState::Connecting));
    ...
    fn connection_states_map_onto_the_room_model() {
        ...
        assert_eq!(map_connection_state(RTCPeerConnectionState::Failed), ConnectionState::Closed);
        assert_eq!(
    ...
    fn keyframe_requests_are_recognised_by_type() {
        let pli: Box<dyn rtc::rtcp::packet::Packet> =
        ...
        assert!(!is_keyframe_request(&report), "receiver reports are not keyframe requests");
    }
    ...
    fn a_negotiated_publisher_codec_wins_over_the_registry() {
        ...
        let track = PublishedTrack::with_codec(
            ParticipantId::new(),
            TrackKind::Camera,
            negotiated,
            None,
    ...
    fn a_track_without_a_negotiated_codec_falls_back_to_the_registry() {
        ...
        assert_eq!(resolve_subscriber_codec(&registry, &audio).unwrap().mime_type, "audio/opus");
        ...
        assert_eq!(resolve_subscriber_codec(&registry, &video).unwrap().mime_type, "video/VP8");
    }
