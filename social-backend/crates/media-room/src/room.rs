...
use social_media_signaling::limits::MAX_PARTICIPANTS_PER_ROOM;
...
impl Default for RoomConfig {
    fn default() -> Self {
        Self { capacity: MAX_PARTICIPANTS_PER_ROOM, auto_subscribe: AutoSubscribe::default() }
    }
}
...
impl std::fmt::Debug for MediaRoom {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MediaRoom").field("room_id", &self.id).finish_non_exhaustive()
    }
}
...
impl MediaRoom {
    ...
    pub fn new(id: Uuid, config: RoomConfig) -> Arc<Self> {
        ...
        Arc::new(Self { id, config, participants: RwLock::new(HashMap::new()), events })
    }
    ...
    pub async fn add_participant(
        ...
        self.emit(RoomEvent::ParticipantJoined { participant: participant.info().await });
        Ok(attached)
    }
    ...
    pub async fn publish_track(
        ...
        self.emit(RoomEvent::TrackPublished { track: track.info().clone() });
...
mod tests {
    ...
    async fn publish(
        ...
        let mime = if kind.is_audio() { "audio/opus" } else { "video/VP8" };
        let track = PublishedTrack::new(participant.id(), kind, mime, None);
        room.publish_track(participant.id(), track.clone()).await.expect("publish");
        track
    }
    ...
    async fn the_first_vertical_slice_two_people_hear_each_other() {
        ...
        room.add_participant(alice.clone()).await.expect("alice joins");
    ...
    async fn video_is_not_auto_subscribed_by_default() {
        ...
        assert!(bob_sink.attached().await.is_empty(), "cellular clients must opt in to video");

        // …until the client asks for it.
        assert!(room.subscribe(bob.id(), camera.id()).await.expect("subscribe"));
    ...
    async fn auto_subscribe_all_catches_video_too() {
        let room = MediaRoom::new(
            Uuid::new_v4(),
            RoomConfig { auto_subscribe: AutoSubscribe::All, ..RoomConfig::default() },
        );
        let (alice, _) = member("Alice");
        let (bob, bob_sink) = member("Bob");
        room.add_participant(alice.clone()).await.expect("join");
    ...
    async fn unpublishing_detaches_every_subscriber_but_not_the_publisher() {
        ...
        room.unpublish_track(alice.id(), TrackKind::Audio).await.expect("unpublish");
        ...
        assert!(!alice_sink.is_closed().await, "unpublishing must not close the transport");
    }
    ...
    async fn unpublishing_something_that_is_not_published_is_a_no_op() {
        ...
        let (alice, _) = member("Alice");
        room.add_participant(alice.clone()).await.expect("join");
        assert_eq!(room.unpublish_track(alice.id(), TrackKind::ScreenShare).await.unwrap(), None);
    }
    ...
    async fn screen_share_has_a_lifecycle_independent_of_the_camera() {
        ...
        room.unpublish_track(alice.id(), TrackKind::ScreenShare).await.expect("stop sharing");

        assert_eq!(bob_sink.attached().await, vec![camera.id().clone()]);
        assert!(alice.published_track(TrackKind::Camera).await.is_some());
        assert!(alice.published_track(TrackKind::ScreenShare).await.is_none());
        let _ = screen;
    }
    ...
    async fn leaving_removes_the_departed_from_everyone_elses_transport() {
        ...
        room.remove_participant(alice.id()).await.expect("alice leaves");

        assert_eq!(room.participant_count().await, 2);
        assert_eq!(bob_sink.attached().await.len(), 0, "alice's track is gone from bob");
        assert_eq!(sarah_sink.attached().await.len(), 1, "only bob remains audible");
        assert!(alice_sink.is_closed().await, "alice's own transport is closed");
    }
    ...
    async fn a_room_becomes_empty_after_the_last_departure() {
        ...
        assert!(room.is_empty().await);
        assert!(room.remove_participant(alice.id()).await.is_none(), "leaving twice is a no-op");
    }

    #[tokio::test]
    async fn capacity_is_enforced() {
        let room =
            MediaRoom::new(Uuid::new_v4(), RoomConfig { capacity: 2, ..RoomConfig::default() });
        let (a, _) = member("A");
        let (b, _) = member("B");
        let (c, _) = member("C");
        ...
        assert!(matches!(room.add_participant(c).await, Err(MediaRoomError::RoomFull)));
    }
