...
use social_domain::room::{self, Room, RoomType};
use social_domain::{CommunityId, DomainError, Permission, RoomId, UserId, now};
use social_community::authorization::apply_room_overrides;
use social_community::CommunityService;
...
impl RoomService {
    ...
    pub fn new(pool: DbPool, communities: CommunityService) -> Self {
        Self { rooms: RoomRepository::new(pool), communities }
    }
    ...
    pub async fn access(&self, room_id: RoomId, user_id: UserId) -> ServiceResult<RoomAccess> {
        ...
        let member = self.communities.member_context(room.community_id, user_id).await?;
        ...
        Ok(RoomAccess { room, member, permissions })
    }
    ...
    pub async fn create(
        ...
        let context = self.communities.member_context(community_id, user_id).await?;
            ...
            && (limit <= 0 || limit > room::MEDIA_ROOM_MAX_PARTICIPANTS)
        {
            return Err(ServiceError::Domain(DomainError::invalid(
                "max_participants",
                format!("must be between 1 and {}", room::MEDIA_ROOM_MAX_PARTICIPANTS),
            )));
        }

        // A cap on a text room is meaningless and would confuse clients.
        let max_participants =
            if input.room_type.is_media() { input.max_participants } else { None };
    ...
    pub async fn list(
        ...
        let member = self.communities.member_context(community_id, user_id).await?;
    ...
    pub async fn update(
        ...
        let name = input.name.as_deref().map(room::validate_room_name).transpose()?;

        Ok(self
            .rooms
            .update(room_id, name.as_deref(), input.topic.as_deref(), input.position, input.max_participants)
            .await?)
    }
