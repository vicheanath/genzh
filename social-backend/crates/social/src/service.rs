...
use social_domain::social::{self, Friendship, FriendshipStatus};
use social_domain::{DomainError, UserId};
...
impl SocialService {
    ...
    pub fn new(pool: DbPool) -> Self {
        Self { repository: SocialRepository::new(pool) }
    }
    ...
    pub async fn request_friend(
        ...
        if self.repository.blocked_either_way(requester_id, addressee_id).await? {
            return Err(ServiceError::denied("blocked"));
        }

        if let Some(existing) = self.repository.find_between(requester_id, addressee_id).await? {
            return match existing.status {
                // Accepting an outstanding request by requesting back is the
                // behaviour users expect, and it avoids a stuck pair.
                FriendshipStatus::Pending if existing.addressee_id == requester_id => self
                    .respond_to_request(requester_id, existing.requester_id, true)
                    .await,
    ...
    pub async fn respond_to_request(
        ...
        let status =
            if accept { FriendshipStatus::Accepted } else { FriendshipStatus::Declined };
    ...
    pub async fn friends(&self, user_id: UserId) -> ServiceResult<Vec<UserId>> {
        let rows = self.repository.list_by_status(user_id, FriendshipStatus::Accepted).await?;
    ...
    pub async fn pending_requests(&self, user_id: UserId) -> ServiceResult<Vec<Friendship>> {
        let rows = self.repository.list_by_status(user_id, FriendshipStatus::Pending).await?;
        Ok(rows.into_iter().filter(|f| f.addressee_id == user_id).collect())
    }
