...
use social_domain::message::{self, Message, ReactionTally};
use social_domain::{DomainError, MessageId, Permission, RoomId, UserId, now};
use social_infrastructure::{DbPool, ServiceError, ServiceResult};
use social_room::RoomService;
...
impl MessagingService {
    ...
    pub fn new(pool: DbPool, rooms: RoomService) -> Self {
        Self { messages: MessageRepository::new(pool), rooms }
    }
    ...
    pub async fn history(
        ...
        Ok(self.messages.list(room_id, before, message::clamp_page_size(limit)).await?)
    }
    ...
    pub async fn react(
        ...
        self.messages.add_reaction(message_id, user_id, &reaction).await?;
    ...
    pub async fn unreact(
        ...
        let reaction = message::validate_reaction(reaction)?;
        if !self.messages.remove_reaction(message_id, user_id, &reaction).await? {
