...
use genzh_media_core::ice::IceServer;
...
impl ClientMessage {
    ...
    pub fn allowed_before_join(&self) -> bool {
        matches!(
            self,
            ClientMessage::Join { .. } | ClientMessage::Ping | ClientMessage::Leave
        )
    }
...
pub fn decode_client_message(raw: &str) -> Result<ClientMessage, ProtocolError> {
    if raw.len() > crate::limits::MAX_MESSAGE_BYTES {
        return Err(ProtocolError::TooLarge {
            limit: crate::limits::MAX_MESSAGE_BYTES,
        });
    }
...
mod tests {
    ...
    fn oversized_frames_are_rejected_without_parsing() {
        let huge = format!(
            r#"{{"type":"offer","target":"publisher","sdp":"{}"}}"#,
            "v".repeat(crate::limits::MAX_MESSAGE_BYTES)
        );
        assert!(matches!(
    ...
    fn sdp_direction_is_enforced_per_connection() {
        let ok_offer = ClientMessage::Offer {
            target: PeerTarget::Publisher,
            sdp: "v=0".into(),
        };
        let ok_answer = ClientMessage::Answer {
            target: PeerTarget::Subscriber,
            sdp: "v=0".into(),
        };
        ...
        let bad_offer = ClientMessage::Offer {
            target: PeerTarget::Subscriber,
            sdp: "v=0".into(),
        };
        let bad_answer = ClientMessage::Answer {
            target: PeerTarget::Publisher,
            sdp: "v=0".into(),
        };
        assert!(matches!(
    ...
    fn server_events_flatten_into_the_envelope() {
        let event = RoomEvent::SpeakingStopped {
    ...
    fn only_speaking_events_are_shed_under_back_pressure() {
        ...
        assert!(
            ServerMessage::event(RoomEvent::SpeakingStarted { participant_id: p }).is_droppable()
        );
        assert!(
            !ServerMessage::event(RoomEvent::ParticipantLeft { participant_id: p }).is_droppable()
        );
        assert!(
