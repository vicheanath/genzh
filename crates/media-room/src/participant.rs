...
use genzh_media_signaling::limits::MAX_TRACKS_PER_PARTICIPANT;
...
impl Participant {
    ...
    pub async fn shutdown(&self) {
        ...
        self.update_state(|s| s.connection = ConnectionState::Closed)
            .await;
        self.sink.close().await;
    }

    /// Public view for participant lists and join payloads.
    pub async fn info(&self) -> genzh_media_core::events::ParticipantInfo {
        ...
        genzh_media_core::events::ParticipantInfo {
    ...
    impl SubscriberSink for RecordingSink {
        async fn attach(&self, track: Arc<PublishedTrack>) -> MediaRoomResult<()> {
            self.calls
                .lock()
                .await
                .push(SinkCall::Attach(track.id().clone()));
            Ok(())
        }
        ...
        async fn detach(&self, track_id: &TrackId) -> MediaRoomResult<()> {
            self.calls
                .lock()
                .await
                .push(SinkCall::Detach(track_id.clone()));
            Ok(())
        }
...
mod tests {
    ...
    async fn subscribing_attaches_exactly_once() {
        ...
        assert!(
            !bob.subscribe(track.clone()).await.expect("idempotent"),
            "already subscribed"
        );
    ...
    async fn a_participant_never_subscribes_to_their_own_track() {
        ...
        assert!(
            sink.attached().await.is_empty(),
            "loopback would echo the speaker"
        );
    }
    ...
    async fn shutdown_detaches_everything_and_closes_the_transport() {
        ...
        assert!(
            sink.attached().await.is_empty(),
            "every subscription must be detached"
        );
