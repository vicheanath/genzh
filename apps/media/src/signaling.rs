...
use genzh_media_core::vad::{SpeakingTransition, VadMode};
use genzh_media_room::participant::Participant;
use genzh_media_room::{MediaRoom, MediaRoomError, ParticipantPeers, PeerEvent, PeerEvents};
use genzh_media_signaling::limits::{
...
async fn handle_client(
    ...
    match message {
        ...
        ClientMessage::Camera { enabled } => {
            participant
                .update_state(|s| s.camera_enabled = enabled)
                .await;
        ...
        ClientMessage::ScreenShare { enabled } => {
            participant
                .update_state(|s| s.screen_sharing = enabled)
                .await;
            room.emit(if enabled {
            ...
            if !enabled {
                let _ = room
                    .unpublish_track(participant_id, TrackKind::ScreenShare)
                    .await;
            }
            Ok(())
        }
...
async fn handle_peer_event(
    ...
    match event {
        ...
        PeerEvent::ConnectionState { target, state } => {
            use genzh_media_room::sfu::connection_state_is_terminal;
            tracing::debug!(target = target.as_str(), ?state, "peer connection state");

            participant
                .update_state(|s| s.connection = genzh_media_room::sfu::map_connection_state(state))
                .await;
...
async fn reply_error(tx: &mut Sender, error: &MediaRoomError) -> Result<(), MediaError> {
    send(
        tx,
        &ServerMessage::error(error.code(), error.client_message()),
    )
    .await
}
